...
This model was implemented for use with a controlled switch in ATP (`SW_TACS`—a TACS-controlled Type-13 switch with GIFU control). The switch control signal is generated by an ATP MODELS routine provided in [`DE.mod`](./DE.mod).
...
Right Bottom -> Models -> Type94 -> THEV and edit the file using [wang.mod](./wang.mod)

The model was compared with [2], called LDM.

## Leader Progression Model (LPM) - CIGRE

Another model was developed by CIGRE [3] and is described more clearly in the updated publication [4]. The present description is based on the tables and mathematical formulations presented in [4]. In this model, the streamer process is considered fully developed when the voltage across the insulation satisfies the following condition:
...
| Configuration | Polarity | $k_1$ [m²/V²/s] | $E_0$ [kV/m] |
|---|:---:|:---:|:---:|
| Air gaps, post insulators, and long-rod insulators | + | $0.8 \times 10^{-6}$ | 600 |
| Air gaps, post insulators, and long-rod insulators | − | $1.0 \times 10^{-6}$ | 670 |
| Disc insulators | + | $1.2 \times 10^{-6}$ | 520 |
| Disc insulators | − | $1.3 \times 10^{-6}$ | 600 |

### ATP 
Like the Wang model, the CIGRE model is implemented using an ATP MODELS file, available here: [`CIGRE.mod`](./CIGRE.mod). The model uses the Type-94 Thevenin interface in ATP.

To add the model in ATPDraw, right-click the workspace and select:

`MODELS → Type 94 → THEV`

Then, edit the component and import the [`CIGRE.mod`](./CIGRE.mod) file.

The implemented model was validated through comparison with the model presented in [2], where it is referred to LDM.
...
[3] CIGRE SC 33 (now C4) WG 33.01, “Guide to procedures for estimating the lightning performance of  transmission lines”, Technical Brochure No. 63, 1991. Reissued with this brochure.

[4] CIGRE SC C4 WG C4.23, "Procedures for Estimating the Lightning Performance of Transmission Lines - New Aspects," Technical Brochure No. 839, June 2021.
